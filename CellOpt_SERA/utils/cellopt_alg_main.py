...
class OptAlgorithm:
    def __init__(self, runid, original_cell_name, score=100000):
        ...
        self.param_offset_dict = OrderedDict()
    ...
    def status(self):
        ...
        print("  Param list: ")
        for param in self.param_dict:
            print("\t\t" + param + " = " + str(self.param_dict[param]) + "\t|\t" + str(self.param_offset_dict[param]))
    ...
    def init_param_offset(self):
        for param in cou.parsDict:
            ...
            self.param_offset_dict[param] = param_offset
            info("Initiated offset of param %s to %e" % (param, param_offset))

    # def update_param_offset(self, param, score_diff, direction):
    #     prev_param_offset = self.param_offset_dict[param]
    #     new_param_offset = ((TARGET_GAP_DIFF_PER_STEP / score_diff) * (1 + prev_param_offset)) - 1
    #     self.param_offset_dict[param] = new_param_offset
    #     print(
    #         "Updated offset of param %s from %f to %f due to score_diff =%f" %
    #         (param, prev_param_offset, new_param_offset, score_diff))

    def update_param_offset(self, param, text="info"):
        # debug(str(self.param_offset_dict[param]))
        # debug(str(self.param_offset_dict[param] / 2))
        self.param_offset_dict[param] = self.param_offset_dict[param] / 2
        if text == "info":
            info("Updated offset of param " + param + " from " + str(self.param_offset_dict[param]) + " to " + str(self.param_offset_dict[param] / 2))

    def is_offset_valid(self, param):
        offset = self.param_offset_dict[param]
        min = cou.parsDict[param].min
        max = cou.parsDict[param].max
        value = self.param_dict[param]
        if value - offset < min or value + offset > max:
            debug("Invalid param " + param + " offset (BoundaryError). Reducing offset. ")
            return False
        else:
            return True
    ...
    def generate_sera_netlist(self, mod_param, sign):
        ...
        while not self.is_offset_valid(mod_param):
            self.update_param_offset(mod_param)

        for param in self.param_dict:
            if param == mod_param:
                param_val = self.param_dict[param] + sign * self.param_offset_dict[param]
                line = '+ ' + param + '=' + \
                    str(param_val)
                sera_obj.param_dict[param] = param_val
            else:
                line = '+ ' + param + '=' + str(self.param_dict[param])
...
def run_opt_sera(runid, original_cell_name):        # Optimizer for SERA minimization
    ...
    while not stop_flag:
        ...
        for param in algorithm.param_dict:
            ...
            args = []
            args.append(algorithm.generate_sera_netlist(param, 1))      # Positive offset
            args.append(algorithm.generate_sera_netlist(param, -1))     # Negative offset

            # Launch as parallel mp worker pool
            ...
            algorithm.param_iteration += 1
        if abs(algorithm.best_score - current_score) < 0.001 * current_score:
            stop_flag = True
        algorithm.descend_iteration += 1
        info("Item " + original_cell_name + " has finished running in SERA Minimization mode.")
        algorithm.status()
        sys.stdout.flush()


def run_opt_ext(runid, original_cell_name, original_score):    # Optimizer backbone for external optimizing algorithm 
    ...
    while True:
        path = os.path.join(RUNSPACE, 'ext_' + str(algorithm.descend_iteration) + '.csv')
...
def run_opt_algorithm(runid, original_cell_name, original_gap):     # General purpose optimizer
    ...
    while fine_tune_itr_cnt < fine_tune_iterations:
        ...
        if not is_better:
            ...
            print(
                ...
                fine_tune_itr_cnt)
            for param in algorithm.param_offset_dict:
                param_offset = algorithm.param_offset_dict[param]
                param_new_offset = param_offset / 2
                algorithm.param_offset_dict[param] = param_new_offset
        ...
        while is_better:
            ...
            for param in algorithm.param_dict:
                ...
                if param not in algorithm.param_offset_dict:
                    print("Missing offset info for param %s , quitting" % param)
                    exit()

                mod_pos = algorithm.param_offset_dict[param]
                mod_neg = algorithm.param_offset_dict[param]

                mod_positive = algorithm.generate_mod_netlist(param, mod_pos, "POS")
                mod_negative = algorithm.generate_mod_netlist(param, -1 * mod_neg, "NEG")
                ...
                while wait_for_completion:
                    ...
                    time.sleep(1)
                    status_pos = os.path.exists(RUNSPACE + "/" + mod_positive + "/" + mod_positive + "_datasheet.txt")
                    status_neg = os.path.exists(RUNSPACE + "/" + mod_negative + "/" + mod_negative + "_datasheet.txt")
    ...
    src_path = RUNSPACE + '/' + algorithm.best_score_id + '/' + algorithm.best_score_id + FILE_EXT
    dst_path = OPT_OUT + '/' + runid + '/' + algorithm.master_cell + '__' + str(algorithm.best_score) + FILE_EXT
    shutil.copy2(src_path, dst_path)
    # Copy datasheet to opt_put/datasheets
    src_path = RUNSPACE + '/' + algorithm.best_score_id + '/' + algorithm.best_score_id + '_datasheet.txt'
    dst_path = OPT_OUT + '/' + runid + '/datasheets/' + algorithm.master_cell + '__' + str(algorithm.best_score) + '_datasheet.txt'
    shutil.copy2(src_path, dst_path)
