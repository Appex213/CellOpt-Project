...
class OptAlgorithm:
    def __init__(self,runid,original_cell_name,gap=100000):
        ...
        self.param_dict = parser.get_sp_params(OPT_IN+'/'+runid+'/'+original_cell_name+'.sp')
        self.noparam_netlist = get_noparam_netlist(OPT_IN+'/'+runid+'/'+original_cell_name+'.sp')
    ...
    def generate_mod_netlist(self,mod_param,modifier):
        file_name = self.original_cell_name+'_'+str(self.global_iteration)+'_'+str(self.local_iteration)
        if modifier == MOD_POS:
        ...
        for param in self.param_dict:
            if param == mod_param:
                line = '+ '+param+'='+str(self.param_dict[param]*modifier)
            else:
    ...
    def liberate(self,cell):
        cmd_str = "$CELL_OPT_ROOT_DIR/utils/launch_char_cell.sh " + cell +' '+ self.master_cell
...
def run_opt_algorithm(runid,original_cell_name,original_gap):
        ...
        sys.stdout = open(LOGS+'/'+algorithm.original_cell_name+'.log','w')     # Redirect output from main stdout to logfile
        ...
        while is_better:
            ...
            for param in algorithm.param_dict:
                param_improving = True
                same_param_counter = 0
                while param_improving and same_param_counter < 5:
                    param_improving = False
        ...
        src_path = RUNSPACE+'/'+algorithm.best_gap_id+'/'+algorithm.best_gap_id+'.sp'
        dst_path = OPT_OUT+'/'+runid+'/'+algorithm.master_cell+'__'+str(algorithm.best_gap)+'.sp'
        shutil.copy2(src_path,dst_path)
        # Copy datasheet to opt_put/datasheets
        src_path = RUNSPACE+'/'+algorithm.best_gap_id+'/'+algorithm.best_gap_id+'_datasheet.txt'
        dst_path = OPT_OUT+'/'+runid+'/datasheets/'+algorithm.master_cell+'__'+str(algorithm.best_gap)+'_datasheet.txt'
        shutil.copy2(src_path,dst_path)
...
def opt_start(cell_name):
    ...
    runid = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+' Alternate Mode'
    print('OptAlgorithm started on '+runid)
    print('+++ALTERNATE ALGORITHM MODE+++')
    ...
    for point in points:
        ...
        point_netlist = open(OPT_IN+'/'+runid+'/'+file_name+'.sp','w')
    ...
    print("OptAlgorithm finished running on "+datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))		



#run_opt_algorithm(sys.argv[1],sys.argv[1].split('__')[1])
opt_start(sys.argv[1])
