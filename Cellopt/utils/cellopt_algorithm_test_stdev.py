...
def run_opt_algorithm(runid, original_cell_name,original_gap):
    ...
    gap_type = "STDEV" # run in standard deviation minimizatuion mode (opose to max-min gap)
            ...
            if timed_out:
                ...
                if status_pos:      # =Status_neg timed out
                    ...
                    gap_pos = parser.get_gap(
                        RUNSPACE + "/" + mod_positive + "/" + mod_positive + "_datasheet.txt",gap_type)
                    ...
                    gap_neg = parser.get_gap(
                        RUNSPACE + "/" + mod_negative + "/" + mod_negative + "_datasheet.txt",gap_type)
...
run_opt_algorithm('test',sys.argv[1],1000)
# opt_start('FA')
