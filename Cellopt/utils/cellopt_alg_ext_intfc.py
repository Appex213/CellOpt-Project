...
class OptAlgorithm:
    ...
    def udate_param_offset(self, param, gap_diff, direction):
        prev_param_offset = self.params_offset_dic[param]
        new_param_offset = ((TARGET_GAP_DIFF_PER_STEP / gap_diff) * (1 + prev_param_offset)) - 1
        self.params_offset_dic[param] = new_param_offset
        print("Updated offset of param %s from %f to %f due to gap_diff =%f" %
              (param, prev_param_offset, new_param_offset, gap_diff))
...
def run_opt_ext(runid, original_cell_name, original_gap):
    ...
    while True:
        ...
        with open(path, 'w', buffering=0) as file:
            ...
            for param in algorithm.param_dict:
                ...
                mod_negative = algorithm.generate_mod_netlist(param, mod_neg, "NEG")
                mod_neutral = algorithm.generate_mod_netlist(param, 0, "NTRL")

                # Datasheet paths
                path_pos = os.path.join(RUNSPACE, mod_positive, mod_positive + "_datasheet.txt")
                path_neg = os.path.join(RUNSPACE, mod_negative, mod_negative + "_datasheet.txt")
                path_ntrl = os.path.join(RUNSPACE, mod_neutral, mod_neutral + "_datasheet.txt")
...
def run_opt_algorithm(runid, original_cell_name, original_gap):
    ...
    fine_tune_iterations = 10  # when there is no improvmenrt scale down offset by fine_tune_factor for fine_tune_iterations
    fine_tune_itr_cnt = 0
    is_better = True
    ...
    while fine_tune_itr_cnt < fine_tune_iterations:
        ...
        if not is_better:
            fine_tune_itr_cnt += 1
            print("INFO: Upon non-improvmed descend iteration , reducing offsets and entering fine tune iteration %d" % fine_tune_itr_cnt)
            for param in algorithm.params_offset_dic:
                ...
                print("Updated offset of param %s from %e to %e" % (param, param_offset, param_new_offset))
                ...
                if timed_out:
                    if status_pos:      # =Status_neg timed out
                        ...
                        if gap_pos < algorithm.best_gap:
                            ...
                            # algorithm.udate_param_offset(param,gap_diff=algorithm.best_gap-gap_pos , direction="POS")
                        ...
                        if gap_neg < algorithm.best_gap:
                            ...
                            # algorithm.udate_param_offset(param,gap_diff=algorithm.best_gap-gap_neg , direction="NEG")
